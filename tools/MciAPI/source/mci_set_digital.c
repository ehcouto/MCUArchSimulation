...
#define S_FUNCTION_NAME  mci_set_digital
...
#include <Windows.h>
#include <string.h>
typedef uint8 (*mcisetdigital_type)(MOTOR_ENUM_TYPE motor, MCI_DO_TYPE read_data, uint8 value);
mcisetdigital_type MciSetDigital = NULL;

#define WRITE_DATA	0
#define GET_WRITE_DATA(S)     (ssGetSFcnParam(S,WRITE_DATA))

#define WRITE_DATA_CMD	1
#define GET_WRITE_DATA_CMD(S)     (ssGetSFcnParam(S,WRITE_DATA_CMD))
/* Error handling
 * --------------
 *
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetSFcnParamTunable(S,WRITE_DATA,false);
    ssSetSFcnParamTunable(S,WRITE_DATA_CMD,false);
...
#define MDL_INITIALIZE_CONDITIONS   /* Change to #undef to remove function */
  ...
  static void mdlInitializeConditions(SimStruct *S)
  {
	  ...
	  {
		  MciSetDigital = mci->methods[MCI_SET_DIG];
		  if (MciSetDigital == NULL)
		  {
...
{
    real_T       *y = ssGetInputPortSignal(S,0);
    int write_data;
    MciStruct* mci;
    ...
    write_data = strtol( mxArrayToString(GET_WRITE_DATA(S)), NULL, 10 );
    if ( MciSetDigital != NULL)
        MciSetDigital(MOTOR0, write_data, *y); 
}
...
static void mdlRTW(SimStruct *S)
{
    ...
    buflen = mxGetN(GET_WRITE_DATA_CMD(S))*sizeof(mxChar)+1;
    write_data = mxMalloc(buflen);
    status = mxGetString((GET_WRITE_DATA_CMD(S)), write_data, buflen);
    
    if (!ssWriteRTWParamSettings(S, 1,
                                     SSWRITE_VALUE_STR, "write_data", write_data))
    {
