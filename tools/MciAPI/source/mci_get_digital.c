...
#define S_FUNCTION_NAME  mci_get_digital
...
#include <Windows.h>
#include <string.h>
typedef sint32 (*mcigetdigital_type)(MOTOR_ENUM_TYPE motor, MCI_DI_TYPE read_data);
mcigetdigital_type MciGetDigital = NULL;

#define READ_DATA	0
#define GET_READ_DATA(S)     (ssGetSFcnParam(S,READ_DATA))

#define READ_DATA_CMD	1
#define GET_READ_DATA_CMD(S)     (ssGetSFcnParam(S,READ_DATA_CMD))
/* Error handling
 * --------------
 *
...
#define MDL_INITIALIZE_CONDITIONS   /* Change to #undef to remove function */
  ...
  static void mdlInitializeConditions(SimStruct *S)
  {
	  ...
	  {
		  MciGetDigital = mci->methods[MCI_GET_DIG];
		  if (MciGetDigital == NULL)
		  {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    read_data = strtol( mxArrayToString(GET_READ_DATA(S)), NULL, 10 );
    if ( MciGetDigital != NULL)
        *y = MciGetDigital(MOTOR0, read_data); 
}
