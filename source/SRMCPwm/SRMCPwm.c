...
#define PWM_OUTPUT_OFF()            SIM__SET_PWM_STATUS_OFF()
#define PWM_OUTPUT_ON()             SIM__SET_PWM_STATUS_ON()
...
static BOOL_TYPE SRMCPwm__FaultState(void);
static BOOL_TYPE SRMCPwm__FaultFlagState(void);
static void SRMCPwm__ClearFaultFlag(void);
//=====================================================================================================================
//-------------------------------------- Public Functions -------------------------------------------------------------
//=====================================================================================================================
 ...
 */
void SRMCPwm__Initialize(unsigned short int pwm_frequency, unsigned short int dead_time_ns)
{
 ...
 */
unsigned short int SRMCPwm__GetMaxPwm(void)
{
    return(MaxPwm);
}
 ...
 * @param pwm_c
 */
void SRMCPwm__LoadAllPwmValues(unsigned short int pwm_a, unsigned short int pwm_b, unsigned short int pwm_c)
{
    SIM_DUTY_A = (float) pwm_a / SIM_PWM_MODULUS;
    SIM_DUTY_B = (float) pwm_b / SIM_PWM_MODULUS;
    SIM_DUTY_C = (float) pwm_c / SIM_PWM_MODULUS;
}
 ...
 */
unsigned short int SRMCPwm__GetPwmFrequencyHz(void)
{
	return (PwmFrequency);
}
 ...
 */
unsigned short int SRMCPwm__GetDeadtimeNanosec(void)
{
...
unsigned char SRMCPwm__GetHwFaultStatus(void)
{
    ...
    temp = SRMCPWM__GET_HW_FAULT_STATUS;
...
void SRMCPwm__Handler250us(void)
{
    ...
    if(temp1 == SRDATA_OK)                          // If data is not corrupted
    {
        ...
        (temp1 == SR_MCPWM_WAITING_TO_CHECK) )      // Checks if it already finished a check or not started
        {
            ...
            if ((SRMCPwm__FaultState() == SR_MCPWM_SHUTDOWN_ACTIVE_POLARITY)||(SRMCPwm__FaultFlagState() != 0))
            {                                       // Checks SD pin
 ...
 */
void SRMCPwm__StartDiagIfAvailable(unsigned long timeout_to_zero, unsigned long timeout_to_one)
{
 ...
 */
SRMCPWM_DIAG_ERROR_TYPE SRMCPwm__GetDiagFeedback(void)
{
    SRMCPWM_DIAG_ERROR_TYPE status;

    status = SRMCPWM__GET_DIAGNOSTIC_FEEDBACK;
